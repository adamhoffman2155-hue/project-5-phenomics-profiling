...
from src.clustering import evaluate_clustering, run_hdbscan, run_umap
from src.data_loader import generate_synthetic_data, validate_data
from src.embeddings import compute_embedding_quality, tvn_normalize
from src.pathway_analysis import build_pathway_cluster_matrix, enrich_cluster_genes
...
def main():
    ...
    parser.add_argument(
        "--n-perturbations", type=int, default=300, help="Number of synthetic perturbations"
    )
    ...
    logger.info(
        f"  Data: {embeddings.shape[0]} perturbations, {embeddings.shape[1]}-dim embeddings"
    )
    ...
    logger.info(
        f"  Embedding quality: variance={quality['mean_variance']:.4f}, sparsity={quality['sparsity']:.4f}"
    )
